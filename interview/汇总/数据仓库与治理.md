...
deduped: 14
generated: 2026-07-23T17:24:10.946137
---
...
1. [金融信贷离线仓库为什么要分 ODS、DWD、DWS、ADS？](#金融信贷离线仓库为什么要分 ODS、DWD、DWS、ADS？)
2. [如何区分离线指标、实时状态、口径解释、血缘追溯和敏感导出问题？](#如何区分离线指标、实时状态、口径解释、血缘追溯和敏感导出问题？)
3. [离线指标血缘和实时告警血缘有什么区别？](#离线指标血缘和实时告警血缘有什么区别？)
4. [日报 + 实时告警摘要为什么必须区分离线日期和实时窗口？](#日报 + 实时告警摘要为什么必须区分离线日期和实时窗口？)
5. [如何设计空分区、实时延迟、告警误报和口径冲突这类 bad case？](#如何设计空分区、实时延迟、告警误报和口径冲突这类 bad case？)
6. [为什么实时窗口和离线日报指标不能混着解释？](#为什么实时窗口和离线日报指标不能混着解释？)
7. [AI 应用上线后需要监控哪些指标？](#AI 应用上线后需要监控哪些指标？)
8. [指标抽取和维度抽取有什么区别？](#指标抽取和维度抽取有什么区别？)
9. [指标字典和 Schema Catalog 有什么区别？](#指标字典和 Schema Catalog 有什么区别？)
10. [为什么需要分层？（30min）](#为什么需要分层？（30min）)
11. [请讲讲分层数据仓库架构设计与 DDL中的ODS 层：原始数据 1:1 镜像（1h）](#请讲讲分层数据仓库架构设计与 DDL中的ODS 层：原始数据 1_1 镜像（1h)
12. [请讲讲分层数据仓库架构设计与 DDL中的DDL 写作规范（30min）](#请讲讲分层数据仓库架构设计与 DDL中的DDL 写作规范（30min）)
13. [为什么简单的"通过/不通过"不够用？（20min）](#为什么简单的_通过_不通过_不够用？（20min）)
14. [请举例说明项目中的扣分制实现（1h）如何实现？](#请举例说明项目中的扣分制实现（1h）如何实现？)
15. [请讲讲数据质量体系 — 扣分制 vs 二元判定中的动手练习（1.5h）](#请讲讲数据质量体系 — 扣分制 vs 二元判定中的动手练习（1.5h）)
16. [请讲讲维度建模 + 宽表设计中的**核心概念**：粒度（30min）](#请讲讲维度建模 + 宽表设计中的__核心概念__：粒度（30min）)
17. [请讲讲维度建模 + 宽表设计中的项目宽表：从 3 张 DWD 到 1 张 DWS（1h）](#请讲讲维度建模 + 宽表设计中的项目宽表：从 3 张 DWD 到 1 张 DWS)
18. [请讲讲维度建模 + 宽表设计中的动手练习（2.5h）](#请讲讲维度建模 + 宽表设计中的动手练习（2.5h）)
19. [请讲讲数据血缘 + SchemaRegistry 元数据管理中的没有血缘的数仓 = 没有地图的城市（20min）](#请讲讲数据血缘 + SchemaRegistry 元数据管理中的没有血缘的数仓)
20. [请讲讲数据血缘 + SchemaRegistry 元数据管理中的SchemaRegistry：数据仓库的"目录服务"（1h）](#请讲讲数据血缘 + SchemaRegistry 元数据管理中的SchemaRe)
21. [请讲讲PII 脱敏 + DDL 规范 + 合规中的脱敏不是"全删"——是"保留该保留的"（30min）](#请讲讲PII 脱敏 + DDL 规范 + 合规中的脱敏不是_全删_——是_保留该)
22. [请讲讲PII 脱敏 + DDL 规范 + 合规中的DDL 写作规范（1h）](#请讲讲PII 脱敏 + DDL 规范 + 合规中的DDL 写作规范（1h）)
23. [请讲讲分区策略 + 数据产品设计中的分区是数仓性能的基石（40min）](#请讲讲分区策略 + 数据产品设计中的分区是数仓性能的基石（40min）)
24. [请讲讲分区策略 + 数据产品设计中的ADS 数据产品：不是"把 DWS 改个名"（1h）](#请讲讲分区策略 + 数据产品设计中的ADS 数据产品：不是_把 DWS 改个名_)
25. [请讲讲综合项目 — 为在线教育平台设计完整数仓中的业务理解（30min）](#请讲讲综合项目 — 为在线教育平台设计完整数仓中的业务理解（30min）)
26. [请谈谈设计任务（2h）怎么设计？](#请谈谈设计任务（2h）怎么设计？)
