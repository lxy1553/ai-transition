...
1. [当⼤模型API响应延迟超过1秒时，前端可以采取哪些优化策略保证⽤户体验？](#当⼤模型API响应延迟超过1秒时，前端可以采取哪些优化策略保证⽤户体验？)
2. [当⼤模型上下⽂窗⼝扩展到100万token时，哪些现有业务场景可能发⽣质变？](#当⼤模型上下⽂窗⼝扩展到100万token时，哪些现有业务场景可能发⽣质变？)
3. [请举例说明假设在电商系统中，哪些功能适合直接使⽤⼤模型完成，哪些需要结合⼯程化⼿段？](#请举例说明假设在电商系统中，哪些功能适合直接使⽤⼤模型完成，哪些需要结合⼯程化⼿)
4. [假设请你设计⼀个医疗问诊系统，如何平衡AI幻觉带来的⻛险与效率提升？需要哪些技术⼿段？](#假设请你设计⼀个医疗问诊系统，如何平衡AI幻觉带来的⻛险与效率提升？需要哪些技术)
5. [模拟面试里为什么一定要准备 bad case 和安全问题？](#模拟面试里为什么一定要准备 bad case 和安全问题？)
6. [FastAPI 在 AI 应用服务化里一般承担什么角色？](#FastAPI 在 AI 应用服务化里一般承担什么角色？)
7. [AI API 服务为什么需要配置、日志和异常处理？](#AI API 服务为什么需要配置、日志和异常处理？)
8. [调用外部 API 或 LLM API 时要怎么保证稳定性？](#调用外部 API 或 LLM API 时要怎么保证稳定性？)
9. [为什么 AI 应用要从脚本重构成后端服务？](#为什么 AI 应用要从脚本重构成后端服务？)
10. [Docker 化解决了 AI 应用交付里的什么问题？](#Docker 化解决了 AI 应用交付里的什么问题？)
11. [为什么配置和密钥不能写死在代码里？](#为什么配置和密钥不能写死在代码里？)
12. [请举例说明一个让你记住一辈子的例子（20min）如何实现？](#请举例说明一个让你记住一辈子的例子（20min）如何实现？)
13. [请举例说明从代码看 PIT 正确性（1h）如何实现？](#请举例说明从代码看 PIT 正确性（1h）如何实现？)
14. [请讲讲PIT 样本构建 — 时间泄漏是 AI 工程师的第一课中的动手练习（1.5h）](#请讲讲PIT 样本构建 — 时间泄漏是 AI 工程师的第一课中的动手练习（1.5)
15. [请说说原始事件 vs 特征向量（20min）](#请说说原始事件 vs 特征向量（20min）)
16. [请讲讲特征工程 — 从事件流到特征向量的三种模式中的三种特征构造模式（1.5h）](#请讲讲特征工程 — 从事件流到特征向量的三种模式中的三种特征构造模式（1.5h）)
17. [为什么不能只用模型？（20min）](#为什么不能只用模型？（20min）)
18. [请举例说明核心代码：`inference_pipeline.py` 的 execute()（1h）如何实现？](#请举例说明核心代码：`inference_pipeline.py` 的 exec)
19. [请讲讲规则 + 模型融合 — 四层决策架构中的动手练习（1.5h）](#请讲讲规则 + 模型融合 — 四层决策架构中的动手练习（1.5h）)
20. [请讲讲模型评估 + 线上监控 + 自动熔断中的模型不是训完就结束了（20min）](#请讲讲模型评估 + 线上监控 + 自动熔断中的模型不是训完就结束了（20min）)
21. [请讲讲模型评估 + 线上监控 + 自动熔断中的上线前评估：四个核心指标（1h）](#请讲讲模型评估 + 线上监控 + 自动熔断中的上线前评估：四个核心指标（1h）)
22. [请讲讲生产级降级 + 容错设计中的动手练习（1.5h）](#请讲讲生产级降级 + 容错设计中的动手练习（1.5h）)
23. [请讲讲★ 参考答案中的NL2SQL：让业务人员用自然语言查数仓（1.5h）](#请讲讲★ 参考答案中的NL2SQL：让业务人员用自然语言查数仓（1.5h）)
24. [请讲讲★ 参考答案中的RAG：让 LLM 基于项目文档回答（1h）](#请讲讲★ 参考答案中的RAG：让 LLM 基于项目文档回答（1h）)
25. [请讲讲★ 参考答案中的LangGraph：多步骤 AI 工作流（40min）](#请讲讲★ 参考答案中的LangGraph：多步骤 AI 工作流（40min）)
26. [请讲讲★ 参考答案中的动手练习（1.5h）](#请讲讲★ 参考答案中的动手练习（1.5h）)
27. [请讲讲可解释性 + 综合项目：智能客服质检系统中的SHAP 可解释性（1h）](#请讲讲可解释性 + 综合项目：智能客服质检系统中的SHAP 可解释性（1h）)
...
## 5. 模拟面试里为什么一定要准备 bad case 和安全问题？
...
## 8. 调用外部 API 或 LLM API 时要怎么保证稳定性？
...
## 12. 请举例说明一个让你记住一辈子的例子（20min）如何实现？
...
## 14. 请讲讲PIT 样本构建 — 时间泄漏是 AI 工程师的第一课中的动手练习（1.5h）
...
## 16. 请讲讲特征工程 — 从事件流到特征向量的三种模式中的三种特征构造模式（1.5h）
...
## 18. 请举例说明核心代码：`inference_pipeline.py` 的 execute()（1h）如何实现？
...
## 19. 请讲讲规则 + 模型融合 — 四层决策架构中的动手练习（1.5h）
...
## 20. 请讲讲模型评估 + 线上监控 + 自动熔断中的模型不是训完就结束了（20min）
...
## 21. 请讲讲模型评估 + 线上监控 + 自动熔断中的上线前评估：四个核心指标（1h）
