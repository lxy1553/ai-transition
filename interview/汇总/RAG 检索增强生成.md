...
1. [RAG 的完整流程是怎么样的？](#RAG的完整流程是怎么样的)
2. [什么⾃查询？为什么在 RAG 中需要⾃查询？](#什么查询为什么在RAG中需要查询)
3. [什么是 RAG 中的分块？为什么需要分块？](#什么是RAG中的分块为什么需要分块)
4. [在 RAG 中，常⻅的分块策略有哪些？分别有什么区别？](#在RAG中常的分块策略有哪些分别有什么区别)
5. [在 RAG 中的 Embedding 嵌⼊是什么？](#在RAG中的Embedding嵌是什么)
6. [什么是 GPTCache？](#什么是GPTCache)
7. [使⽤LangChain时，如何实现多路召回结果的动态权重分配？](#使LangChain时如何实现多路召回结果的动态权重分配)
8. [当发现RAG系统召回结果与⽤户query意图不匹配时，有哪些可能的改进⽅向？](#当发现RAG系统召回结果与户query意图不匹配时有哪些可能的改进向)
9. [使⽤LangChain实现RAG系统时，如何处理PDF⽂档中的表格数据召回问题？](#使LangChain实现RAG系统时如何处理PDF档中的表格数据召回问题)
10. [什么是 RAG 中的 Rerank？具体需要怎么做？](#什么是RAG中的Rerank具体需要怎么做)
11. [什么混合检索？在基于⼤模型的应⽤开发中，混合检索主要解决什么问题？](#什么混合检索在基于模型的应开发中混合检索主要解决什么问题)
12. [在 RAG 应⽤中为了优化检索精度，其中的数据清洗和预处理怎么做？](#在RAG应中为了优化检索精度其中的数据清洗和预处理怎么做)
13. [在 RAG 中，你如何选择 Embedding Model 嵌⼊模型，需要考虑哪些因素？](#在RAG中你如何选择EmbeddingModel嵌模型需要考虑哪)
14. [在 RAG 中，索引流程中的⽂档解析你们怎么做的？](#在RAG中索引流程中的档解析你们怎么做的)
15. [你都了解哪些向量数据库？如何选型？](#你都了解哪些向量数据库如何选型)
16. [向量数据库中的 HNSW、LSH、PQ 分别是什么意思？](#向量数据库中的HNSWLSHPQ分别是什么意思)
17. [向量数据库中的 ANN 是什么？为什么需要⽤它？](#向量数据库中的ANN是什么为什么需要它)
18. [什么是 Advanced RAG？](#什么是AdvancedRAG)
19. [什么是 Modular RAG？](#什么是ModularRAG)
20. [你有多个知识库，做 RAG 的时候，怎么保证查询效率和准确性兼容，并尽可能减少幻觉？](#你有多个知识库做RAG的时候怎么保证查询效率和准确性兼容并尽可能减少幻)
21. [生产环境里的 RAG 系统一般怎么设计？](#生产环境里的RAG系统一般怎么设计)
22. [为什么生产级 RAG 要分离线入库和在线问答两条链路？](#为什么生产级RAG要分离线入库和在线问答两条链路)
23. [RAG 文档切分时怎么设计 chunk？](#RAG文档切分时怎么设计chunk)
24. [向量检索、关键词检索和混合检索有什么区别？](#向量检索关键词检索和混合检索有什么区别)
25. [RAG 离线链路里的混合检索怎么设计，RRF 是什么？](#RAG离线链路里的混合检索怎么设计RRF是什么)
26. [rerank 在 RAG 里解决什么问题？](#rerank在RAG里解决什么问题)
27. [RAG 为什么要做引用来源和可追溯？](#RAG为什么要做引用来源和可追溯)
28. [RAG 知识库更新和版本管理怎么做？](#RAG知识库更新和版本管理怎么做)
29. [RAG 知识入库和索引构建在生产环境里怎么设计？](#RAG知识入库和索引构建在生产环境里怎么设计)
30. [RAG 在线问答为什么必须返回引用来源？](#RAG在线问答为什么必须返回引用来源)
31. [RAG 召回质量差时，你会怎么排查和优化？](#RAG召回质量差时你会怎么排查和优化)
32. [RAG 问答 API 在生产环境里应该怎么设计？](#RAG问答API在生产环境里应该怎么设计)
33. [一个 RAG 项目怎样包装成可展示、可复盘的作品？](#一个RAG项目怎样包装成可展示可复盘的作品)
34. [Query rewrite 在 RAG 里解决什么问题，生产里怎么控制风险？](#Queryrewrite在RAG里解决什么问题生产里怎么控制风险)
35. [RAG 项目 README 至少应该包含哪些内容？](#RAG项目README至少应该包含哪些内容)
36. [为什么 RAG 项目演示不能只展示成功问题，还要展示 bad case？](#为什么RAG项目演示不能只展示成功问题还要展示badcase)
37. [Day 17 到 Day 20 的 RAG 项目完整链路是什么？](#Day17到Day20的RAG项目完整链路是什么)
38. [本地 RAG 项目当前有哪些限制，后续可以怎么优化？](#本地RAG项目当前有哪些限制后续可以怎么优化)
39. [用户原始问题为什么不一定适合直接检索？](#用户原始问题为什么不一定适合直接检索)
40. [怎么判断 query rewrite 是否真的改善了召回质量？](#怎么判断queryrewrite是否真的改善了召回质量)
41. [RAG 测试集应该怎么构建，评估哪些指标？](#RAG测试集应该怎么构建评估哪些指标)
42. [为什么评估 RAG 不能只看最终答案？](#为什么评估RAG不能只看最终答案)
43. [expected_sources 在 RAG 测试集中有什么作用？](#expected_sources在RAG测试集中有什么作用)
44. [RAG 系统什么时候应该拒答？](#RAG系统什么时候应该拒答)
45. [RAG 幻觉通常怎么产生，怎么降低？](#RAG幻觉通常怎么产生怎么降低)
46. [RAG 系统怎么做权限控制和敏感信息保护？](#RAG系统怎么做权限控制和敏感信息保护)
47. [RAG 的 citations 也会有安全风险吗？](#RAG的citations也会有安全风险吗)
48. [生产 RAG 怎么控制 token 成本和响应延迟？](#生产RAG怎么控制token成本和响应延迟)
49. [RAG 系统里缓存应该怎么设计，为什么不能只按问题文本缓存？](#RAG系统里缓存应该怎么设计为什么不能只按问题文本缓存)
50. [为什么 RAG 不是 top-k 越大越好？](#为什么RAG不是top-k越大越好)
51. [你会怎么准备一个 RAG 项目的 5 分钟演示？](#你会怎么准备一个RAG项目的5分钟演示)
52. [RAG 项目演示前应该做哪些稳定性检查？](#RAG项目演示前应该做哪些稳定性检查)
53. [如何把一个 RAG Demo 包装成可投递项目？](#如何把一个RAGDemo包装成可投递项目)
54. [数据清洗和字段校验为什么会影响后续 RAG 或数据问答质量？](#数据清洗和字段校验为什么会影响后续RAG或数据问答质量)
55.  [为什么口径解释问题应该走 RAG，而不是直接生成 SQL？](#为什么口径解释问题应该走RAG而不是直接生成SQL)
56. [为什么这套项目应该被表达成"金融信贷离线/实时仓库 + Agent"而不是单独的 RAG Demo？](#为什么这套项目应该被表达成_金融信贷离线_实时仓库Agent_而不是单独的)
57. [什么是 RAG？详细描述一个完整 RAG 系统的详细工作流程？](#什么是RAG详细描述一个完整RAG系统的详细工作流程)
58. [你使用 RAG 给大模型一个输入，系统是怎样的工作流程？](#你使用RAG给大模型一个输入系统是怎样的工作流程)
59. [如何润色用户的 Query（Query Rewrite）？目的是什么？](#如何润色用户的QueryQueryRewrite目的是什么)
60. [什么是多路召回？具体怎么做？](#什么是多路召回具体怎么做)
61. [RAG 检索优化策略有哪些？](#RAG检索优化策略有哪些)
62. [了解哪些更复杂的 RAG 范式？](#了解哪些更复杂的RAG范式)
63.  [在什么场景下，你会选择使用图数据库来增强传统的向量检索？](#在什么场景下你会选择使用图数据库来增强传统的向量检索)
64. [如何规避 RAG 系统中大模型的幻觉？](#如何规避RAG系统中大模型的幻觉)
65. [怎么量化你的 RAG 效果？](#怎么量化你的RAG效果)
66. [RAG 知识库如何实现动态与持续更新？](#RAG知识库如何实现动态与持续更新)
67. [大模型的 RAG 主要用来解决什么问题？](#大模型的RAG主要用来解决什么问题)
68. [在实际落地中，你觉得 RAG 最难的地方是哪里？](#在实际落地中你觉得RAG最难的地方是哪里)
69. [相比直接微调 LLM，RAG 解决了什么问题？微调和 RAG 各自的优劣势是什么？](#相比直接微调LLMRAG解决了什么问题微调和RAG各自的优劣势是什么)
70. [RAG 中的文档是怎么存的？粒度是多大？详细说说文档切割（Chunking）策略？](#RAG中的文档是怎么存的粒度是多大详细说说文档切割Chunking策略)
71. [怎么规避语义被切割掉的问题？](#怎么规避语义被切割掉的问题)
72. [在 RAG 中 Embedding 究竟是什么？如何选择和评估一个 Embedding 模型？](#在RAG中Embedding究竟是什么如何选择和评估一个Embedd)
73. [Embedding 有哪几种算法你了解过吗？](#Embedding有哪几种算法你了解过吗)
74. [什么是向量数据库？有没有做过向量数据库的对比选型？](#什么是向量数据库有没有做过向量数据库的对比选型)
75. [讲讲你用的向量数据库？数据量级是多大？性能如何？遇到过性能瓶颈吗？](#讲讲你用的向量数据库数据量级是多大性能如何遇到过性能瓶颈吗)
