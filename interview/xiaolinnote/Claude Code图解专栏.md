...
1. [1. Claude Code /powerup 教程：18 个官方互动课程全解析](#1.-claude-code-/powerup-教程：18-个官方互动课程全解析)
2. [技术栈](#技术栈)
3. [代码规范](#代码规范)
4. [项目结构](#项目结构)
5. [2. Claude Code 使用教程：新手入门必学的基础技巧](#2.-claude-code-使用教程：新手入门必学的基础技巧)
6. [07｜ 每次都要重复解释项目？用 CLAUDE.md(http://CLAUDE.md)(#07-每次都要重复解释项目-用-claude-md)](#07｜-每次都要重复解释项目用-claude.md(http://claude.md)(#07-每次都要重复解释项目-用-claude-md))
7. [技术栈](#技术栈)
8. [代码规范](#代码规范)
9. [项目结构](#项目结构)
10. [3. Anthropic 40 万次会话研究：用好 AI 编程的关键不是会写代码](#3.-anthropic-40-万次会话研究：用好-ai-编程的关键不是会写代码)
11. [4. CLAUDE.md 指南：Claude Code 的项目记忆该怎么写？](#4.-claude.md-指南：claude-code-的项目记忆该怎么写)
12. [01｜CLAUDE.md(http://CLAUDE.md) 到底是个什么东西？(#01-claude-md-到底是个什么东西)](#01｜claude.md(http://claude.md)-到底是个什么东西(#01-claude-md-到底是个什么东西))
13. [04｜CLAUDE.md(http://CLAUDE.md) 不只是一个文件(#04-claude-md-不只是一个文件)](#04｜claude.md(http://claude.md)-不只是一个文件(#04-claude-md-不只是一个文件))
14. [1. Project Overview](#1.-project-overview)
15. [2. Commands](#2.-commands)
16. [3. Architecture](#3.-architecture)
17. [4. Conventions](#4.-conventions)
18. [5. Hard Constraints](#5.-hard-constraints)
19. [6. Gotchas](#6.-gotchas)
20. [5. Claude Code Skill 揭秘：Skill 真的只是一份 markdown 吗？](#5.-claude-code-skill-揭秘：skill-真的只是一份-markdown-吗)
21. [6. Claude Code 大型代码库实战：百万行代码怎么扛得住？](#6.-claude-code-大型代码库实战：百万行代码怎么扛得住)
22. [Q2：CLAUDE.md(http://CLAUDE.md) 到底写多长合适？写了 1000 行 Claude 反而变笨？(#q2-claude-md-到底写多长合适-写了-1000-行-claude-反而变笨)](#q2：claude.md(http://claude.md)-到底写多长合适写了-1000-行-claude-反而变笨(#q2-claude-md-到底写多长合适-写了-1000-行-claude-反而变笨))
23. [7. SDD 规约驱动开发实战：在 Claude Code 里跑通先规约后编码](#7.-sdd-规约驱动开发实战：在-claude-code-里跑通先规约后编码)
24. [8. grill-me 使用指南：写代码前，先让 Claude Code 审问你的需求](#8.-grill-me-使用指南：写代码前先让-claude-code-审问你的需求)
25. [9. superpowers vs grill-me：同一个需求实测，差距到底在哪？](#9.-superpowers-vs-grill-me：同一个需求实测差距到底在哪)
26. [10. Claude Fable 5 系统提示词详解：1600 行泄漏 prompt 里的工程干货](#10.-claude-fable-5-系统提示词详解：1600-行泄漏-prompt-里的工程干货)
27. [claudebehavior      （产品信息 / 安全 / 语气 / 用户关怀…）](#claudebehavior------（产品信息-/-安全-/-语气-/-用户关怀…）)
28. [memorysystem](#memorysystem)
29. [mcpappsuggestions](#mcpappsuggestions)
30. [computeruse          （skills / artifacts…）](#computeruse----------（skills-/-artifacts…）)
31. [searchinstructions](#searchinstructions)
32. [Tool Definitions      ← 一个一个工具的完整定义](#tool-definitions------←-一个一个工具的完整定义)
33. [network / filesystem configuration](#network-/-filesystem-configuration)
34. [11. Claude Code 上下文管理图解：Compact 压缩机制怎么实现？](#11.-claude-code-上下文管理图解：compact-压缩机制怎么实现)
35. [12. Claude Code 主循环 Query 图解：一轮对话是怎么跑起来的？](#12.-claude-code-主循环-query-图解：一轮对话是怎么跑起来的)
36. [13. Claude Code 代码检索图解：为什么用 grep 而不用 RAG？](#13.-claude-code-代码检索图解：为什么用-grep-而不用-rag)
37. [14. Claude Code 多 Agent 图解：SubAgent 实现机制怎么做？](#14.-claude-code-多-agent-图解：subagent-实现机制怎么做)
38. [1. Your Role](#1.-your-role)
39. [15. Claude Code 源码拆解：51 万行泄漏代码里的架构设计](#15.-claude-code-源码拆解：51-万行泄漏代码里的架构设计)
40. [16. Claude Code 记忆机制图解：为什么不用向量数据库？](#16.-claude-code-记忆机制图解：为什么不用向量数据库)
41. [四、静态层：CLAUDE.md(http://CLAUDE.md) 的六个层级(#四、静态层-claude-md-的六个层级)](#四、静态层：claude.md(http://claude.md)-的六个层级(#四、静态层-claude-md-的六个层级))
