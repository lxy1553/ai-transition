...
1. [1. 请举例说明一个让你记住一辈子的例子（20min）如何实现？](#1.-请举例说明一个让你记住一辈子的例子（20min）如何实现)
2. [2. 请举例说明从代码看 PIT 正确性（1h）如何实现？](#2.-请举例说明从代码看-pit-正确性（1h）如何实现)
3. [3. 请讲讲PIT 样本构建 — 时间泄漏是 AI 工程师的第一课中的动手练习（1.5h）](#3.-请讲讲pit-样本构建-—-时间泄漏是-ai-工程师的第一课中的动手练习（1.5h）)
4. [4. 请讲讲PIT 样本构建 — 时间泄漏是 AI 工程师的第一课中的跨业务思考（30min）](#4.-请讲讲pit-样本构建-—-时间泄漏是-ai-工程师的第一课中的跨业务思考（30min）)
5. [5. 请讲讲PIT 样本构建 — 时间泄漏是 AI 工程师的第一课中的今日要点](#5.-请讲讲pit-样本构建-—-时间泄漏是-ai-工程师的第一课中的今日要点)
6. [6. 请讲讲PIT 样本构建 — 时间泄漏是 AI 工程师的第一课中的检查清单](#6.-请讲讲pit-样本构建-—-时间泄漏是-ai-工程师的第一课中的检查清单)
7. [7. 请说说原始事件 vs 特征向量（20min）](#7.-请说说原始事件-vs-特征向量（20min）)
8. [8. 请讲讲特征工程 — 从事件流到特征向量的三种模式中的三种特征构造模式（1.5h）](#8.-请讲讲特征工程-—-从事件流到特征向量的三种模式中的三种特征构造模式（1.5h）)
9. [9. 请讲讲特征工程 — 从事件流到特征向量的三种模式中的动手练习（1.5h）](#9.-请讲讲特征工程-—-从事件流到特征向量的三种模式中的动手练习（1.5h）)
10. [10. 请讲讲特征工程 — 从事件流到特征向量的三种模式中的跨业务思考（30min）](#10.-请讲讲特征工程-—-从事件流到特征向量的三种模式中的跨业务思考（30min）)
11. [11. 请讲讲特征工程 — 从事件流到特征向量的三种模式中的今日要点](#11.-请讲讲特征工程-—-从事件流到特征向量的三种模式中的今日要点)
12. [12. 为什么不能只用模型？（20min）](#12.-为什么不能只用模型（20min）)
13. [13. 请举例说明核心代码：inferencepipeline.py 的 execute()（1h）如何实现？](#13.-请举例说明核心代码：inferencepipeline.py-的-execute()（1h）如何实现)
14. [14. 请讲讲规则 + 模型融合 — 四层决策架构中的动手练习（1.5h）](#14.-请讲讲规则-+-模型融合-—-四层决策架构中的动手练习（1.5h）)
15. [15. 请讲讲规则 + 模型融合 — 四层决策架构中的跨业务思考（30min）](#15.-请讲讲规则-+-模型融合-—-四层决策架构中的跨业务思考（30min）)
16. [16. 请讲讲模型评估 + 线上监控 + 自动熔断中的模型不是训完就结束了（20min）](#16.-请讲讲模型评估-+-线上监控-+-自动熔断中的模型不是训完就结束了（20min）)
17. [17. 请讲讲模型评估 + 线上监控 + 自动熔断中的上线前评估：四个核心指标（1h）](#17.-请讲讲模型评估-+-线上监控-+-自动熔断中的上线前评估：四个核心指标（1h）)
18. [18. 请讲讲模型评估 + 线上监控 + 自动熔断中的熔断器：不让坏模型继续害人（40min）](#18.-请讲讲模型评估-+-线上监控-+-自动熔断中的熔断器：不让坏模型继续害人（40min）)
19. [19. 请讲讲模型评估 + 线上监控 + 自动熔断中的动手练习（1.5h）](#19.-请讲讲模型评估-+-线上监控-+-自动熔断中的动手练习（1.5h）)
20. [20. 请举例说明三层降级代码精读（1h）如何实现？](#20.-请举例说明三层降级代码精读（1h）如何实现)
21. [21. 请讲讲生产级降级 + 容错设计中的动手练习（1.5h）](#21.-请讲讲生产级降级-+-容错设计中的动手练习（1.5h）)
22. [22. 请讲讲生产级降级 + 容错设计中的跨业务思考（30min）](#22.-请讲讲生产级降级-+-容错设计中的跨业务思考（30min）)
23. [23. 请讲讲★ 参考答案中的NL2SQL：让业务人员用自然语言查数仓（1.5h）](#23.-请讲讲★-参考答案中的nl2sql：让业务人员用自然语言查数仓（1.5h）)
24. [24. 请讲讲★ 参考答案中的RAG：让 LLM 基于项目文档回答（1h）](#24.-请讲讲★-参考答案中的rag：让-llm-基于项目文档回答（1h）)
25. [25. 请讲讲★ 参考答案中的LangGraph：多步骤 AI 工作流（40min）](#25.-请讲讲★-参考答案中的langgraph：多步骤-ai-工作流（40min）)
26. [26. 请讲讲★ 参考答案中的动手练习（1.5h）](#26.-请讲讲★-参考答案中的动手练习（1.5h）)
27. [27. 请讲讲可解释性 + 综合项目：智能客服质检系统中的SHAP 可解释性（1h）](#27.-请讲讲可解释性-+-综合项目：智能客服质检系统中的shap-可解释性（1h）)
28. [28. 请讲讲可解释性 + 综合项目：智能客服质检系统中的综合项目：智能客服质检系统（3h）](#28.-请讲讲可解释性-+-综合项目：智能客服质检系统中的综合项目：智能客服质检系统（3h）)
29. [29. 请讲讲可解释性 + 综合项目：智能客服质检系统中的自评表（30min）](#29.-请讲讲可解释性-+-综合项目：智能客服质检系统中的自评表（30min）)
30. [30. 请讲讲可解释性 + 综合项目：智能客服质检系统中的一周回顾](#30.-请讲讲可解释性-+-综合项目：智能客服质检系统中的一周回顾)
