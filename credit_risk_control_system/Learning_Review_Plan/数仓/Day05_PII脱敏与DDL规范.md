...
```python
# 电商数据的敏感字段，为每个字段选择脱敏策略

def mask_收货人姓名(name: str) -> str:
    """参考 DataMasker.mask_name"""
    pass

def mask_收货手机号(phone: str) -> str:
    """参考 DataMasker.mask_phone"""
    pass

def generalize_收货地址(address: str) -> str:
    """
    参考 DataMasker.mask_phone 的思路（保留部分信息）

    泛化层级:
    完整: "北京市朝阳区望京街道XX小区3号楼502"
    Level 1(区级): "北京市朝阳区" ← 可分析区域分布
    Level 2(市级): "北京市" ← 仍可分析城市分布
    Level 3(省级): "北京"  ← 仍可分析地域分布
    Level 4(哈希): hash(address) ← 只能去重
    """
    pass

def hash_支付卡号(card_no: str) -> str:
    """参考 DataMasker.hash_user_id"""
    pass
```
