"""Day 2 - Python 基础补齐主入口。

这个文件把验证器、清洗器、日志分析器三个工具串起来跑一遍。
目的不是做复杂业务，而是练习“一个主入口 + 多个工具模块”的代码组织方式。
以后做 RAG、NL2SQL 或 API 服务时，也会用类似方式把入口层和工具层分开。
...
def demo_validator():
    ...
    验证器负责提前拦截明显不合法的数据。真实项目里如果不先做校验，
    脏数据会继续进入清洗、入库、模型调用等后续流程，排查成本会更高。
    """
    print("\n" + "=" * 70)
    print("1. 数据验证器演示")
    print("=" * 70)
...
def demo_cleaner():
    ...
    清洗器负责把输入整理成统一格式。真实数据经常有多余空格、大小写不一致、
    薪资格式不统一等问题，不先清洗就会影响统计、检索和后续判断。
    """
    print("\n" + "=" * 70)
    print("2. 数据清洗器演示")
    print("=" * 70)
...
def demo_log_analyzer():
    ...
    日志分析器模拟生产排查问题的基本动作：读日志、识别级别、统计错误。
    以后做 API 或 RAG 服务时，也需要靠日志判断请求是否成功、哪里失败。
    """
    print("\n" + "=" * 70)
    print("3. 日志分析器演示")
    print("=" * 70)
...
def main():
    """按固定顺序运行 Day 2 的三个练习模块。

    主函数只负责组织流程，不把所有细节写在这里。
    这样以后要替换某个工具类时，不会影响整个入口结构。
    """
    print("=" * 70)
