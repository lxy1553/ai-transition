...
| 链路 | 作用 | 什么时候发生 |
|------|------|--------------|
| 离线入库链路 | 把原始文档处理成可检索的知识库 | 文档新增、更新、定时同步时 |
| 在线问答链路 | 用户提问后检索资料并生成回答 | 每次用户请求时 |
...
| 文档类型 | 推荐切法 | 原因 |
|----------|----------|------|
| 指标口径 | 按指标切 | 一个 chunk 包含一个完整指标定义 |
| 表结构 | 按表切 | 一个 chunk 包含一张表的字段说明 |
| FAQ | 按问答对切 | 问题和答案天然成对 |
| SQL 规范 | 按规则条目切 | 每条规则可以独立召回 |
| README | 按标题切 | 保留项目功能和运行说明结构 |
...
| 存储 | 存什么 | 用途 |
|------|--------|------|
| 向量库 | chunk 的 embedding | 相似度检索 |
| 原文库 | chunk 原文 | 组装上下文、展示引用 |
| 元数据 | 文档名、权限、版本、更新时间 | 过滤、审计、追踪 |
...
| 风险 | 例子 | 控制方式 |
|------|------|----------|
| 幻觉 | 编造不存在的字段 | 必须基于引用回答 |
| 越权 | 普通用户看到财务指标 | 权限过滤 |
| 口径错误 | 使用旧版指标定义 | 文档版本管理 |
| 召回失败 | 没找到正确表结构 | 优化 chunk 和检索 |
| 成本过高 | 上下文塞太多 | 控制 top-k 和 token |
| 延迟过高 | 检索和模型都慢 | 缓存、降级、异步 |
...
| 对比项 | 学习版 RAG | 生产级 RAG |
|--------|------------|------------|
| 文档来源 | 少量 Markdown | 多来源、多格式、多版本 |
| 文档治理 | 简单整理 | 去重、脱敏、权限、版本 |
| 检索 | 向量 top-k | 混合检索 + 权限过滤 + 重排 |
| 生成 | 直接调用模型 | Prompt 模板、重试、降级 |
| 回答 | 返回文本 | 答案 + 引用 + 置信度 + 风险 |
| 安全 | 基本不考虑 | 敏感信息、越权、审计 |
| 评测 | 手工观察 | 测试集、指标、用户反馈 |
| 运维 | 本地脚本 | 日志、监控、成本、延迟 |
...
> 学习版 RAG 是“检索 + 生成”，生产级 RAG 是“文档治理 + 权限安全 + 混合检索 + 重排 + 上下文控制 + 生成校验 + 监控评测”的完整工程系统。
