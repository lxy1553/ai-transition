...
目标不是“问答”，而是先把资料整理成系统能检索的知识库。
这一步对应生产 RAG 里的文档接入、解析、切分、embedding、索引构建和版本管理。
...
如果没有这一步，用户提问时系统就不知道去哪里找资料。
所以知识入库就是把散落的 Markdown、README、规则说明、项目笔记，
加工成一条条可以检索、可以引用、可以管理的 chunk。
...
| 数据 | 生产常见存储 |
|------|--------------|
| 原始文件 | S3、OSS、COS、MinIO、文件系统 |
| 文档元数据 | PostgreSQL、MySQL |
| chunk 正文 | PostgreSQL、MongoDB、文档库 |
| embedding 向量 | pgvector、Milvus、Qdrant、OpenSearch |
| 权限和版本 | PostgreSQL、权限系统 |
| 入库日志 | ELK、OpenSearch、ClickHouse |
...
|--------------|------------------|
| 读取本地 Markdown | 文档接入 |
| 计算 hash | 判断文档是否变化，支持增量更新 |
| 按标题切 chunk | 文档切分策略 |
| 构建 source、title、status | metadata 管理 |
| 生成模拟向量 | embedding 生成 |
| 写 SQLite | 模拟数据库和向量库存储 |
| 输出 manifest | 入库任务审计和排查 |
...
| 问题 | 影响 | 生产处理方式 |
|------|------|--------------|
| 文档重复入库 | 检索结果重复，浪费存储 | 用 hash 去重 |
| chunk 太大 | 召回不精准，token 成本高 | 按标题、段落、语义边界切分 |
| chunk 太小 | 上下文丢失，模型答不完整 | 设置最小长度和 overlap |
| metadata 缺失 | 无法过滤、引用、审计 | 入库时强制写 doc_id、source、权限、版本 |
| 无权限标签 | 可能泄露内部资料 | 文档和 chunk 都要带权限信息 |
| 无版本字段 | 用户查到旧口径 | 保留 version、updated_at、status |
| 只存向量 | 无法回查原文和来源 | 向量、正文、metadata 分开存 |
...
原因是当前目标是先理解“入库数据结构”和“离线处理链路”。
SQLite 足够模拟文档表、chunk 表、embedding 表和入库任务表。
